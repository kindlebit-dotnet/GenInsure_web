    ...
    public class AccountController : Controller
    {
            ...
            // To enable password failures to trigger account lockout, change to shouldLockout: true
              ...
              switch (result)
              {
                case SignInStatus.Success:
                    ...
                    if (role == "Administrator" || role == "SuperUser" || role == "Agent" || role == "AgentStaff" || role == "Reports" || role == "Finance" || role == "Claim" || role == "Team Leaders" || role == "Zimnata"||role=="Broker" ||role=="Broker Manager")
                    {
        ...
        public async Task<ActionResult> AddUserManagement(UserManagementViewModel model)
        {
            ...
            if (model.Id == 0)
            {
                ...
                {
                    log.WriteLog("exception AddUserManagement:" + ex.Message);
                    TempData["ErrorMsg"] = ex.Message;
                    return RedirectToAction("UserManagementList", "Account");
                }
        ...
        public ActionResult UserManagementList(string msg = "")
        {
            ...
            List<CustomerModel> ListUserViewModel = new List<CustomerModel>();
            var name = "Customer";
            var secondQuery = "select TOP 100 AspNetUsers.Email, AspNetUsers.UserName, Customer.IsActive, Customer.Gender, Customer.DateOfBirth, Customer.Country, Customer.City, Customer.Countrycode, Customer.Id as CustomerId, Customer.Id as Id, AspNetUsers.Id as UserID, Customer.PhoneNumber, Customer.FirstName, Customer.LastName, AspNetRoles.Name, Branch.BranchName, WorkType.Name as WorkDesc ";
            ...
            secondQuery += " left join WorkType on Customer.worktypeId= WorkType.Id where AspNetRoles.Name !='" + name + "' and AspNetRoles.Name != 'Web Customer'   order by Customer.Id desc";
        ...
        public ActionResult SearchUserManagementList(string searchText)
        {
            ...
            if (searchText != null && searchText != "")
            {
                ...
                if (custom.Length == 2)
                {
                    ...
                    var secondQuery = "select AspNetUsers.Email, AspNetUsers.UserName,Customer.IsActive, Customer.Gender, Customer.DateOfBirth, Customer.Country, Customer.City, Customer.Countrycode, Customer.Id as CustomerId, Customer.Id as Id, AspNetUsers.Id as UserID, Customer.PhoneNumber, Customer.IsActive, Customer.FirstName, Customer.LastName, AspNetRoles.Name, Branch.BranchName, WorkType.Name as WorkDesc";
                    secondQuery += " from AspNetUsers join AspNetUserRoles on AspNetUsers.Id = AspNetUserRoles.UserId join AspNetRoles on AspNetUserRoles.RoleId = AspNetRoles.Id join Customer on AspNetUsers.Id = Customer.UserID left join Branch on Branch.Id = Customer.BranchId";
                    secondQuery += " left join WorkType on Customer.worktypeId= WorkType.Id where (FirstName like '%" + searchtext1 + "%' and LastName like '%" + searchtext2 + "%') AND (AspNetRoles.Name != 'Web Customer' and AspNetRoles.Name != 'Customer')  ";
        ...
        [HttpPost]
        public JsonResult ActivateUserManagement(int id)
        {
            string msg = "";
            try
            {
        ...
        [Authorize(Roles = "Staff,Administrator,Renewals,Agent,AgentStaff,Claim,Finance,Team Leaders,ALMSupport")]
        public ActionResult PolicyManagement()
        {
            ...
            //query += " VehicleDetail.Id as VehicleId, VehicleDetail.isLapsed, VehicleDetail.IsActive, VehicleDetail.RenewalDate,  VehicleDetail.LicExpiryDate,VehicleDetail.ALMBranchId, VehicleDetail.RenewPolicyNumber,ReinsuranceCommission,ReinsurancePremium, ReinsuranceAmount ";
            ...
            //    RecieptStatus = GetSummaryStatus(recieptList, x.RenewPolicyNumber, x.Id, x.ALMBranchId),

            //}).ToList();

            List<PolicyListViewModel> list = GetAllPolicy();
        ...
        private List<PolicyListViewModel> GetAllPolicy()
        {
            var recieptList = InsuranceContext.ReceiptHistorys.All().ToList();
            var summaryVehicleList = InsuranceContext.SummaryVehicleDetails.All().ToList();


            List<PolicyListViewModel> list = new List<PolicyListViewModel>();
            ...
            {
                using (SqlCommand cmd = new SqlCommand("sp_getPolicyManagement", myConnection))
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            PolicyListViewModel model = new PolicyListViewModel();
                            model.PolicyId = dr["PolicyId"] == DBNull.Value ? 0 : Convert.ToInt32(dr["PolicyId"]);
                            model.ALMBranchId = dr["ALMBranchId"] == DBNull.Value ? 0 : Convert.ToInt32(dr["ALMBranchId"]);
                            model.PolicyNumber = dr["PolicyNumber"] == DBNull.Value ? "" : Convert.ToString(dr["PolicyNumber"]);
                            model.CustomerName = dr["CustomerName"] == DBNull.Value ? "" : Convert.ToString(dr["CustomerName"]);
                            model.CustomerId = dr["CustomerId"] == DBNull.Value ? 0 : Convert.ToInt32(dr["CustomerId"]);
                            model.PaymentMethod = dr["PaymentMethod"] == DBNull.Value ? "" : Convert.ToString(dr["PaymentMethod"]);
                            model.TotalSumInsured = dr["TotalSumInsured"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["TotalSumInsured"]);
                            model.TotalPremium = dr["TotalPremium"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["TotalPremium"]);
                            model.createdOn = dr["createdOn"] == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(dr["createdOn"]);
                            model.SummaryId = dr["Id"] == DBNull.Value ? 0 : Convert.ToInt32(dr["Id"]);
                            model.RegisterationNumber = dr["RegistrationNo"] == DBNull.Value ? "" : Convert.ToString(dr["RegistrationNo"]);
                            model.Make = dr["Make"] == DBNull.Value ? "" : Convert.ToString(dr["Make"]);
                            model.Model = dr["Model"] == DBNull.Value ? "" : Convert.ToString(dr["Model"]);
                            model.Currency = dr["Currency"] == DBNull.Value ? "" : Convert.ToString(dr["Currency"]);
                            model.VehicleSumInsured = dr["SumInsured"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["SumInsured"]);
                            model.isLapsed = dr["isLapsed"] == DBNull.Value ? false : Convert.ToBoolean(dr["isLapsed"]);
                            model.IsActive = dr["IsActive"] == DBNull.Value ? false : Convert.ToBoolean(dr["IsActive"]);
                            model.VehicleDetailId = dr["VehicleId"] == DBNull.Value ? 0 : Convert.ToInt32(dr["VehicleId"]);
                            model.RenewalDate = GetRenewalDate(dr["RenewalDate"] == DBNull.Value ? "" : Convert.ToString(dr["RenewalDate"]), dr["LicExpiryDate"] == null ? "" : Convert.ToString(dr["LicExpiryDate"]));
                            model.RenewPolicyNumber = dr["RenewPolicyNumber"] == DBNull.Value ? "" : Convert.ToString(dr["RenewPolicyNumber"]);
                            model.BrokerCommission = dr["ReinsuranceCommission"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["ReinsuranceCommission"]);
                            model.AutoFacPremium = dr["ReinsurancePremium"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["ReinsurancePremium"]);
                            model.FacultativeCommission = dr["ReinsuranceCommission"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["ReinsuranceCommission"]);
                            model.FacPremium = dr["ReinsurancePremium"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["ReinsurancePremium"]);
                            model.FacReinsuranceAmount = dr["ReinsuranceAmount"] == DBNull.Value ? 0 : Convert.ToDecimal(dr["ReinsuranceAmount"]);
                            model.PaymentStatus = GetPaymentStatus(recieptList, model.RenewPolicyNumber, model.PolicyId, model.PaymentMethod);
                            model.IsExistOnSummaryVehcile = CheckVehicleExistOnSummaryVehicle(summaryVehicleList, model.VehicleDetailId);
                            model.RecieptStatus = GetSummaryStatus(recieptList, model.RenewPolicyNumber, model.SummaryId, model.ALMBranchId);
        ...
        private List<PolicyListViewModel> GetPolicyBySearch(string searchText)
        {
            var recieptList = InsuranceContext.ReceiptHistorys.All().ToList();
            var summaryVehicleList = InsuranceContext.SummaryVehicleDetails.All().ToList();
            ...
            using (SqlConnection myConnection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_SearchPolicy", myConnection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    myConnection.Open();

                    cmd.Parameters.AddWithValue("@searchField", searchText);
        ...
        public ActionResult RenewPolicyList()
        {
            ...
            var query = "   select top 100  PolicyDetail.PolicyNumber as Policy_Number, VehicleDetail.ALMBranchId,  Customer.PhoneNumber, ";
            ...
            var result = InsuranceContext.Query(query).Select(c => new PolicyListViewModel()
            {
                ...
                LicExpDate = c.LicExpiryDate == null ? "" : Convert.ToDateTime(c.LicExpiryDate).ToShortDateString(),
                RenewalDate = GetRenewalDate(c.RenewalDate == null ? "" : Convert.ToDateTime(c.RenewalDate).ToShortDateString(), Convert.ToDateTime(c.LicExpiryDate).ToShortDateString()),
        ...
        public ActionResult SearchRenewPolicyList(ListPolicy Model)
        {
            ...
            var res = result.Where(c => Convert.ToDateTime(c.RenewalDate) >= Convert.ToDateTime(Model.FromDate) && Convert.ToDateTime(c.RenewalDate) <= Convert.ToDateTime(Model.EndDate)).ToList();
            policylist.listpolicy = res;
        ...
        // GET: Dashboard
        [Authorize(Roles = "Administrator, AgentStaff, Agent, SuperUser,Reports,Finance,Claim,Team Leaders,Zimnata,Broker,Broker Manager")]
        ...
        [Authorize(Roles = "Staff,Administrator,Renewals,Agent,AgentStaff,Finance,Claim,Team Leaders,ALMSupport")]
        public ActionResult SearchPolicy(string searchText)
        {
            ClearRenewSession();
            ...
            //query += " SummaryDetail.TotalSumInsured, SummaryDetail.TotalPremium, SummaryDetail.CreatedOn, SummaryDetail.Id, VehicleDetail.RegistrationNo, ";
            //query += "   VehicleMake.MakeDescription as Make, VehicleModel.ModelDescription as Model, Currency.Name as currency, VehicleDetail.SumInsured, ";
            //query += " VehicleDetail.Id as VehicleId, VehicleDetail.isLapsed, VehicleDetail.IsActive, VehicleDetail.ALMBranchId, VehicleDetail.RenewalDate,  VehicleDetail.LicExpiryDate, VehicleDetail.RenewPolicyNumber,ReinsuranceCommission,ReinsurancePremium, ReinsuranceAmount ";
            //query += " from PolicyDetail join Customer on PolicyDetail.CustomerId = Customer.Id  join VehicleDetail on VehicleDetail.PolicyId= PolicyDetail.Id ";
            //query += " join  SummaryVehicleDetail on VehicleDetail.Id=SummaryVehicleDetail.VehicleDetailsId ";
            //query += "  join SummaryDetail on SummaryDetail.Id = SummaryVehicleDetail.SummaryDetailId   ";
            //query += "  join (select distinct SummaryDetailId from PaymentInformation) as Payment on SummaryDetail.Id= Payment.SummaryDetailId   ";
            ...
            //FacultativeCommission
            return View("PolicyManagement", GetPolicyBySearch(searchText));
        }
        ...
        [Authorize(Roles = "Customer,Web Customer,Staff,Team Leaders,Agent,Renewals,Renewals,Administrator,ALMSupport")]
        ...
        public ActionResult MyPolicies()
        {
            ...
            if (role == "Staff" || role == "Team Leaders" || role == "Agent" || role == "Renewals")
            {
        ...
        public async Task<JsonResult> DisablePolicyWithReason(int VehicleId, string reason)
        {
            try
            {
                ...
                SummaryDetailService.WriteLog(VehicleId.ToString(), DateTime.Now.ToShortDateString(), "DisablePolicyWithReason", reason);
                ...
                SummaryDetailService.WriteLog(_customer.UserID, DateTime.Now.ToShortDateString(), "DisablePolicyWithReason", reason);
               ...
               // InsuranceContext.SmsLogs.Insert(objsmslog);
                ...
                SummaryDetailService.WriteLog(VehicleId.ToString(), DateTime.Now.ToShortDateString(), "DisablePolicyWithReason3", reason);
            ...
            {
                SummaryDetailService.WriteLog(VehicleId.ToString(), DateTime.Now.ToShortDateString(), "DisablePolicyWithReason ex ", ex.Message);
        ...
        public async Task<JsonResult> ActivateVehicle(int VehicleId)
        {
            try
            {
                SummaryDetailService.WriteLog(VehicleId.ToString(), DateTime.Now.ToShortDateString(), "ActivateVehicle");
            ...
            {
                SummaryDetailService.WriteLog(VehicleId.ToString(), DateTime.Now.ToShortDateString(), "ActivateVehicle ex ", ex.Message);
            ...
            foreach (var item in EmailsContant)
            {
                ...
                foreach (var _item in _vehicleList)
                {
                    ...
                    var body = EmailBody2.Replace("##RenewDate##", _item.RenewalDate.Value.ToString("dd/MM/yyyy")).Replace("##path##", filepath)
                        ...
                        .Replace("##Make##", make).Replace("##Model##", model).Replace("##Vrn##",_item.RegistrationNo);
                    try
                    {
