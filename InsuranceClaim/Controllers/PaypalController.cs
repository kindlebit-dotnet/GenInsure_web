...
namespace InsuranceClaim.Controllers
{
    public class PaypalController : Controller
    {
        ...
        {
            string email = "";
            bool _userLoggedin = (HttpContext.User != null) && HttpContext.User.Identity.IsAuthenticated;
            if (_userLoggedin)
            {
                ...
                if (summaryDetial != null)
                {
                    foreach (var item in summaryDetial)
                    {
                        ...
                        else if (vichelDetails != null && vichelDetails.LicenseId != null && vichelDetails.VehicleLicenceFee > 0)
                        {
                            ...
                            //  res = ICEcashService.TPIPolicy(vichelDetails, PartnerToken);
                            var resLic = ICEcashService.LICResult(vichelDetails.LicenseId, PartnerToken);

                            if (resLic.Response != null && resLic.Response.Message.Contains("Partner Token has expired"))
                            {
                                tokenObject = iceCash.getToken();
                                //   tokenObject = (ICEcashTokenResponse)Session["ICEcashToken"];
                                SummaryDetailService.UpdateToken(tokenObject);
                                // tokenObject = service.CheckSessionExpired();
                                PartnerToken = tokenObject.Response.PartnerToken;
                                resLic = ICEcashService.LICResult(vichelDetails.LicenseId, PartnerToken);
                            }


                            if (resLic.Response != null && resLic.Response.Message.Contains("Licence Retrieved"))
                            {
                                //if (res.Response.Status == "Approved")
                                // OTPConfirmationObject optMod = new OTPConfirmationObject();

                                result = resLic.Response.Status;
                                vichelDetails.InsuranceStatus = "Approved";
                                vichelDetails.CoverNote = resLic.Response.PolicyNo;




                                if (resLic.Response.LicenceCert != null && resLic.Response.LicenceCert.Length > 1)
                        ...
                        {
                            result = res.Response==null? "" :res.Response.Message;
                        }
                    }
                }
            }
