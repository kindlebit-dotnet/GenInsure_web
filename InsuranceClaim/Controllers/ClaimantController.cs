    ...
    public class ClaimantController : Controller
    {
        ...
        public ActionResult ClaimantList()
        {
            ...
            objList = (from j in InsuranceContext.ClaimNotifications.All().ToList()
                           //join vd in InsuranceContext.VehicleDetails.All().GroupBy(k=>k.PolicyId).Select(j=>j.OrderByDescending(c => c.TransactionDate)).ToList()
                           // on j.PolicyId equals vd.PolicyId
                       join vd in InsuranceContext.VehicleDetails.All().ToList()
                       on j.PolicyId equals vd.PolicyId


                       join pd in InsuranceContext.PolicyDetails.All().ToList()
                       on j.PolicyId equals pd.Id
                       into rd

                       from rt in rd.DefaultIfEmpty()
                           //join vd in InsuranceContext.VehicleDetails.All() on rt.Id equals vd.PolicyId into vechicle
            ...
            var objclaimlist = objList.GroupBy(x => x.Id).Select(y => y.First()).Where(m=>m.Isactive==true).ToList();
        ...
        public ActionResult Claimstatus()
        {
            ...
            var dt = new List<ClaimsStatusModel>();
            //var query = "SELECT ClaimNotification.PolicyNumber,ClaimRegistration.Id, ClaimRegistrationProviderDetial.ClaimRegistrationId,ClaimAdjustment.ClaimRegisterationId FROM ClaimNotification";
            //    query += " FULL JOIN ClaimRegistration ON  ClaimNotification.PolicyNumber = ClaimRegistration.PolicyNumber";
            //    query += " FULL JOIN ClaimRegistrationProviderDetial ON ClaimRegistration.Id = ClaimRegistrationProviderDetial.ClaimRegistrationId";
            //    query += " FULL JOIN ClaimAdjustment ON ClaimRegistrationProviderDetial.ClaimRegistrationId = ClaimAdjustment.ClaimRegisterationId";

            //---get records from ClaimNotification ----------

            var q1 = "select ClaimNotification.PolicyNumber,ClaimNotification.ClaimantName, ClaimNotification.CreatedOn from ClaimNotification";
            q1 += " join  VehicleDetail on ClaimNotification.PolicyId = VehicleDetail.PolicyId";
            q1 += " join  PolicyDetail on ClaimNotification.PolicyId = PolicyDetail.Id";
            //q1 += " join  VehicleMake on ClaimNotification.ThirdPartyMakeId = VehicleMake.MakeCode";
            //q1 += " join  VehicleModel on ClaimNotification.ThirdPartyModelId = VehicleModel.ModelCode";
            q1 += " where ClaimNotification.IsRegistered=0 AND ClaimNotification.IsDeleted =1 AND  VehicleDetail.IsActive=1";
             var result = InsuranceContext.Query(q1).Select(c => new ClaimDetailsProviderModel
            {
                PolicyNumber=c.PolicyNumber,
            ...
            {
                dt.Add(new ClaimsStatusModel
                { 
                    PolicyNumber= items.PolicyNumber,
                    CreatedOn = Convert.ToDateTime(items.CreatedOn),
                    PolicySteps = 1
                });
            }


            //---get records from ClaimRegistration ----------

            var q2 = "select  ClaimRegistration.Id, ClaimStatus.[Status], PolicyNumber,PaymentDetails, ClaimNumber, PlaceOfLoss, DescriptionOfLoss, ";
            q2 += " EstimatedValueOfLoss, ThirdPartyDamageValue, ClaimStatus, VehicleDetailId, ClaimantName, ";
            q2 += "RegistrationNo, MakeDescription, ModelDescription, TotalProviderFees, ClaimRegistration.CreatedOn from ClaimRegistration join ClaimStatus";
            q2 += " on ClaimRegistration.ClaimStatus = ClaimStatus.Id ";
            q2 += " left join VehicleMake on ClaimRegistration.MakeId = VehicleMake.MakeCode";
            q2 += " left join VehicleModel on ClaimRegistration.ModelId = VehicleModel.ModelCode";

            var result2 = InsuranceContext.Query(q2).Select(c => new ClaimDetailsProviderModel
            {
                PolicyNumber = c.PolicyNumber,
                CreatedOn = Convert.ToString(c.CreatedOn)
            }).ToList();
            ...
            //---get records from ClaimRegistrationProviderDetial ----------

            var q3 = "select PolicyNumber, ClaimNumber, ClaimRegistration.Id as ClaimRegistrationId, ServiceProvider.ServiceProviderName, ";
            q3 += " ServiceProviderType.ProviderType, ClaimRegistrationProviderDetial.CreatedOn, ClaimRegistrationProviderDetial.Id from ClaimRegistrationProviderDetial ";
            q3 += " join ClaimRegistration on ClaimRegistrationProviderDetial.ClaimRegistrationId = ClaimRegistration.id ";
            q3 += " join ServiceProvider on ClaimRegistrationProviderDetial.ServiceProviderId = ServiceProvider.Id ";
            q3 += " join ServiceProviderType on ClaimRegistrationProviderDetial.ServiceProviderTypeId = ServiceProviderType.Id ";
            //Ds
            q3 += " where ClaimRegistrationProviderDetial.IsActive = 1";

            q3 += " order by ClaimRegistrationProviderDetial.Id desc";

            var result3 = InsuranceContext.Query(q3).Select(c => new ClaimDetailsProviderModel
            {
                PolicyNumber = c.PolicyNumber,
                CreatedOn = Convert.ToString(c.CreatedOn)
            }).ToList();
            ...
            //---get records from ClaimAdjustments ----------
            ...
            foreach (var items in ListClaimAdjust)
            {
                dt.Add(new ClaimsStatusModel
                {
                    PolicyNumber = items.PolicyNumber,
            ...
            var objlists = dt.GroupBy(x => x.PolicyNumber).Select(y => y.Last()).ToList();
            //dt= dt.GroupBy(m => m.PolicyNumber);
            return View(objlists.OrderByDescending(m=>m.CreatedOn));
        }
        ...
        {
            ViewBag.servicename = InsuranceContext.ServiceProviderTypes.All().Where(m=>m.Id==4 || m.Id==8 || m.Id==7).ToList();
            ViewBag.providername = InsuranceContext.ServiceProviders.All().Where(m => m.IsDeleted == true).ToList();
        ...
        public JsonResult getServiceProviderName(int? id)
        {
            ...
            if (id == 7)
            {
                jsonResult.Data = Providertype.Where(w => w.ServiceProviderType == 7).Select(y => new { y.Id, y.ServiceProviderName }).ToList();
            }
            if (id == 8)
            {
                jsonResult.Data = Providertype.Where(w => w.ServiceProviderType == 8).Select(y => new { y.Id, y.ServiceProviderName }).ToList();
            }
            if (id == 3)
            {
        ...
        public JsonResult GetCustomername(string txtvalue)
        {
            ...
            try
            {
                ...
                var Policydetaildata = InsuranceContext.PolicyDetails.Single(where: $"PolicyNumber='{txtvalue}'");
                if (Policydetaildata != null || txtvalue != null)
                {
                    if (Policydetaildata != null)
                    {
                         polcyid = Policydetaildata.Id;
                    }
                    var vehicledetaildata = InsuranceContext.VehicleDetails.All().Where(m => m.RegistrationNo == txtvalue).FirstOrDefault();
                    var vehicledetaildta = InsuranceContext.VehicleDetails.All().Where(m => m.PolicyId == polcyid).FirstOrDefault();
                    //Single(where: $"PolicyId='{Policydetaildata.Id}' AND RegistrationNo='{txtvalue}' ");
                    if ((vehicledetaildata != null) || (Policydetaildata != null))
                    {
                        if (vehicledetaildata != null)
                        {
                           isactval = vehicledetaildata.IsActive;
                        }
                        if (Policydetaildata != null)
                        {
                             isact2 = vehicledetaildta.IsActive;
                        }
                        if (isactval == true|| isact2==true)
                        {
                            var customerName = "";
                            var policyNumber = "";
                            var vrn = "";
                            ...
                            if (detail == null)
                            {
                                ...
                                var vehicle = InsuranceContext.VehicleDetails.All().Where(m => m.RenewPolicyNumber == policyNumber || m.RegistrationNo == policyNumber).FirstOrDefault();
                                if (vehicle != null)
                                {
                                ...
                                if (detail != null)
                                {
                                    ...
                                    model.RegistrationNo = vehicledetaildata.RegistrationNo;
                                    model.PolicyNumber = policyNumber;
