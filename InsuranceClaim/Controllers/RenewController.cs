...
namespace InsuranceClaim.Controllers
{
    ...
    public class RenewController : Controller
    {
        ...
        [Authorize(Roles = "Renewals,Administrator,Team Leaders,Agent,ALMSupport")]
        public ActionResult Index(int? vehicleid)
        {
        ...
        public ActionResult SummaryDetail(int summaryDetailId = 0)
        {
          ...
          //  model.RecieptStatus = GetRecieptStatus( vehicle.Id);
        ...
        private string GetRecieptStatus( int vehicleId)
        {
            var vehicleDetail = InsuranceContext.VehicleDetails.Single(vehicleId);
            int almBrachId = vehicleDetail.ALMBranchId;
            string recieptStatus = "";

            AccountController account = new AccountController();

            var summaryVehileDetai = InsuranceContext.SummaryVehicleDetails.Single(where: "VehicleDetailsId=" + vehicleId);

            if(summaryVehileDetai!=null)
            {
                var recieptList = InsuranceContext.ReceiptHistorys.All(where: "SummaryDetailId=" + summaryVehileDetai.SummaryDetailId).ToList();

                recieptStatus = account.GetSummaryStatus(recieptList, vehicleDetail.RenewPolicyNumber, summaryVehileDetai.SummaryDetailId, almBrachId);
            }
            ...
            try
            {
                if (model != null)
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        if (role == "Renewals" || role == "Team Leaders" || role == "Administrator" || role == "Agent" || role == "ALMSupport")
                        {
                        ...
                        if (!string.IsNullOrEmpty(btnSendQuatation))
                        {
                            ...
                            foreach (var item in ListOfVehicles)
                            {
                                ...
                                Summeryofcover += "<tr> <td style='padding: 1px 10px; '> <font size='1'> Transaction Date: </font> </td> <td style='padding: 1px 10px; '> <font size='1'>" + item.TransactionDate.Value.ToString("dd/MM/yyyy") + " </font> </td> </tr>";
        ...
        public async Task<ActionResult> SaveDetailList(Int32 id, string invoiceNumber = "", string paymentId = "")
        {
            ...
            if (Session["RenewVehicleDetails"] != null)
            {
                ...
                Summeryofcover += "<tr> <td style='padding: 1px 10px; '> <font size='1'> Transaction Date: </font> </td> <td style='padding: 1px 10px; '> <font size='1'>" + vehicle.TransactionDate.Value.ToString("dd/MM/yyyy") + " </font> </td> </tr>";
                Summeryofcover += " </table>";
            ...
            try
            {
                ...
                if (vichelDetails != null && vichelDetails.CombinedID != null && vichelDetails.VehicleLicenceFee > 0)
                {
                    ...
                    if (res.Response != null && res.Response.LicExpiryDate != null)
                    {
                        ...
                        vichelDetails.LicExpiryDate = LicExpiryDate;
                    }
                }
